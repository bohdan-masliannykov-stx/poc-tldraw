...
export class Brushing extends StateNode {
  ...
  private startShape() {
    ...
    if (this.initialShape) {
      ...
      if (shape && this.segmentMode === "straight") {
        ...
        const { x, y } = this.editor
          .getPointInShapeSpace(shape, originPagePoint)
          .toFixed();
        ...
        if (this.canClose()) {
          (shapePartial as TLShapePartial<TLDrawShape>).props!.isClosed =
            this.getIsClosed(segments, shape.props.size, shape.props.scale);
        }

        this.editor.updateShapes<TLDrawShape | TLHighlightShape>([
          shapePartial,
        ]);
  ...
  private updateDrawingShape() {
    ...
    const { x, y, z } = this.editor
      ...
      .toFixed();
    const pressure = this.isPenOrStylus
      ? +(inputs.currentPagePoint.z! * 1.25).toFixed(2)
      : 0.5;
    ...
    switch (this.segmentMode) {
      case "starting_straight": {
        ...
        if (hasMovedFarEnough) {
          ...
          if (this.canClose()) {
            (shapePartial as TLShapePartial<TLDrawShape>).props!.isClosed =
              this.getIsClosed(segments, size, scale);
          }
      ...
      case "starting_free": {
        ...
        if (hasMovedFarEnough) {
          ...
          if (this.canClose()) {
            (shapePartial as TLShapePartial<TLDrawShape>).props!.isClosed =
              this.getIsClosed(finalSegments, size, scale);
          }
      ...
      case "straight": {
        ...
        let newPoint = this.editor
          .getPointInShapeSpace(shape, currentPagePoint)
          .toFixed()
          .toJson();
        let didSnap = false;
      ...
      case "free": {
        ...
        if (this.canClose()) {
          (shapePartial as TLShapePartial<TLDrawShape>).props!.isClosed =
            this.getIsClosed(newSegments, size, scale);
        }
        ...
        if (newPoints.length > this.editor.options.maxPointsPerDrawShape) {
          ...
          this.initialShape = structuredClone(
            this.editor.getShape<DrawableShape>(newShapeId)!
          );
          this.mergeNextPoint = false;
          this.lastRecordedPoint = inputs.currentPagePoint.clone();
          this.currentLineLength = 0;
